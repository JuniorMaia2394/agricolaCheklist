import 'package:cheklist/models/tractor_problem.dart';

Map<String, TractorProblem> TRACTOR_PROBLEMS = {
  '1': TractorProblem(
    id: '1',
    title: 'Nível do óleo do motor.',
    imageURL: 'images/card/card_image_1.png',
    status: '',
  ),
  '2': TractorProblem(
    id: '2',
    title: 'Nível de oléo hidraúlico.',
    imageURL: 'images/card/card_image_2.png',
    status: '',
  ),
  '3': TractorProblem(
    id: '3',
    title: 'Nível do óleo da transmissão.',
    imageURL: 'images/card/card_image_3.PNG',
    status: '',
  ),
  '4': TractorProblem(
    id: '4',
    title: 'Nível do óleo do freio de serviço.',
    imageURL: 'images/card/card_image_4.PNG',
    status: '',
  ),
  '5': TractorProblem(
    id: '5',
    title: 'Nível da água do radiador.',
    imageURL: 'images/card/card_image_5.PNG',
    status: '',
  ),
  '6': TractorProblem(
    id: '6',
    title: 'Verificar correia e tensor da ventuinha.',
    imageURL: 'images/card/card_image_6.PNG',
    status: '',
  ),
  '7': TractorProblem(
    id: '7',
    title: 'Água do reservatório do limpador de para-brisa se equipado.',
    imageURL: 'images/card/card_image_7.PNG',
    status: '',
  ),
  '8': TractorProblem(
    id: '8',
    title: 'Solução e fazer limpeza dos polos da bateria.',
    imageURL: 'images/card/card_image_8.jpeg',
    status: '',
  ),
  '9': TractorProblem(
    id: '9',
    title:
        'Capas protetoras de alavancas de cambio, grupo, freios, TDP, TDA e entre outras.',
    imageURL: 'images/card/card_image_9.jpeg',
    status: '',
  ),
  '10': TractorProblem(
    id: '10',
    title: 'Realizar lubrificação a graxa a cada 10 horas \nde trabalho.',
    imageURL: 'images/card/card_image_10.jpg',
    status: '',
  ),
  '11': TractorProblem(
    id: '11',
    title:
        'Suspiros, caixa de marcha, tampa do reservatório de freio, tampa tanque de combustível, tampa do bocal de enchimento do óleo hidráulico/trasmissão, eixo dianteiro e cubos traseiros.',
    imageURL: 'images/card/card_image_11.PNG',
    status: '',
  ),
  '12': TractorProblem(
    id: '12',
    title: 'Pneus e fazer calibragem pelo menos a cada 15 dias.',
    imageURL: 'images/card/card_image_12.PNG',
    status: '',
  ),
  '13': TractorProblem(
    id: '13',
    title: 'Presença de fios soltos.',
    imageURL: 'images/card/card_image_13.PNG',
    status: '',
  ),
  '14': TractorProblem(
    id: '14',
    title: 'Folga nas rodas dianteiras e traseiras.',
    imageURL: 'images/card/card_image_14.PNG',
    status: '',
  ),
  '15': TractorProblem(
    id: '15',
    title: 'Funcionamento dos instrumentos do painel do equipamento.',
    imageURL: 'images/card/card_image_15.PNG',
    status: '',
  ),
  '16': TractorProblem(
    id: '16',
    title:
        'Folga do pedal da embreagem e não deixar o pé sobre o pedal da embreagem após troca de marcha.',
    imageURL: 'images/card/card_image_15.PNG',
    status: '',
  ),
  '17': TractorProblem(
    id: '17',
    title: 'Acionamento das alavancas de levante \nde 3 pontos e VCR.',
    imageURL: 'images/card/card_image_15.PNG',
    status: '',
  ),
  '18': TractorProblem(
    id: '18',
    title: 'Funcionamento das luzes de serviços.',
    imageURL: 'images/card/card_image_15.PNG',
    status: '',
  ),
  '19': TractorProblem(
    id: '19',
    title:
        'Realizar diariamente a drenagem do filtro sedimentador e o filtro de combustível.',
    imageURL: 'images/card/card_image_15.PNG',
    status: '',
  ),
  '20': TractorProblem(
    id: '20',
    title:
        'Fazer inspeção em volta do equiamento(360°) e analisar se há vazamentos em geral, parafusos soltos, tubulações e mangueiras, presença de trincas e ver se tem outros problemas.',
    imageURL: 'images/card/card_image_15.PNG',
    status: '',
  ),
  '21': TractorProblem(
    id: '21',
    title:
        'Equipamento em funcionamento e se todos os controles estão funcionando normalmente.',
    imageURL: 'images/card/card_image_15.PNG',
    status: '',
  ),
  '22': TractorProblem(
    id: '22',
    title:
        'Itens de segurança(Cinto de segurança, retrovisor, buzina e sirene de ré).',
    imageURL: 'images/card/card_image_15.PNG',
    status: '',
  ),
};
